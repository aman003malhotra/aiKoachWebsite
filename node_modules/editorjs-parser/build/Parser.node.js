...
var defaultParsers = {
  ...
  list: function (data) {
    ...
    const items = data.items.reduce(
      (acc, item) => acc + `<li class="text-gray-600">${item}</li>`,
      ""
    );
    return `<${type} class="m-[16px]">${items}</${type}>`;
  },
  ...
  quote: function (data, config) {
    ...
    return `<blockquote class="p-4 my-4 border-l-4 border-gray-300 bg-gray-100 " ${alignment}><p class="text-xl italic font-medium leading-relaxed text-gray-900">${data.text}</p><cite>${data.caption}</cite></blockquote>`;
  },
...
var defaultConfig = {
  ...
  paragraph: {
    pClass: "mb-3 text-gray-600 tracking-wide leading-7",
  },
  code: {
    codeBlockClass: "block whitespace-pre overflow-x-scroll p-4 my-4 bg-gray-800 text-gray-100 max-h-[160px]",
  },
  embed: {
